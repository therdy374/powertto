...
if (isset($_POST['add_newAdmin'])) {
	...
	if (preg_match($koreanPattern, $email)) {
		echo "<script> alert('아이디는 영문,숫자 4자 이상가능합니다.\\n 숫자만은 불가합니다.');</script>";
		exit;
	}

	$check_admin_id = "SELECT * FROM admins WHERE email = '$email'";
	$check_result = mysqli_query($conn, $check_admin_id);

	if (mysqli_num_rows($check_result) > 0) {

		echo "<script> alert('이미 사용중인 아이디입니다.asd')</script>";
		echo "<script> window.location.href = 'index_L4.php?view_admin'; </script>";
	} else {


		$bycrypt_password = password_hash($password, PASSWORD_BCRYPT);

		$query = "INSERT INTO admins (parent_name,name,email,password,phone,verify_status,admin_level,referal_code,referal_points,percentage) 
                        VALUES ('$parent_name','$name','$email','$bycrypt_password','$phone','1','$admin_level','$referal_code','0','$percentage')";
