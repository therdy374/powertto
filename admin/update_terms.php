...
if (isset($_GET['update_terms'])) {
    ...
    $user_id = $_GET['update_terms'];

    $select_query = "SELECT * FROM `terms_condition` WHERE id='$user_id'";
    ...
    if (isset($_POST['update_terms'])) {
        ...
        if ($content == '') {
            ...
            $sql2 = "UPDATE `terms_condition` SET `content`='$content' WHERE `id` = '$user_id'";
            $res_query = mysqli_query($conn, $sql2);

            if ($res_query) {
                echo '<script>alert("The content announcement is now updated!")</script>';
                echo "<script>window.open('index.php?terms_condition','_self')</script>";
            } else {
...
<body>
    <div class="col-md-12 col-xl-12">
        <div class="bg-secondary rounded h-100 p-4">
            <h6 class="mb-4">Update Terms & Condition
                <a href="index.php?terms_condition" class="btn btn-primary btn-sm float-end">Back</a>
            </h6>
            ...
            <form action="" method="POST" class="mb-2">
                ...
                <input type="submit" value="Update Terms & Condition" name="update_terms" class="bg-primary border-0 rounded py-2 px-3 text-light">
            </form>
        </div>
    </div>
