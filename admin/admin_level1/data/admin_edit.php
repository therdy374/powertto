...
if (isset($_GET['admin_edit'])) {
	...
	if (isset($_POST['agent_update'])) {
        ...
        if ($name == '' || $email == '' || $phone == '' || $percentage == '' || $admin_level == '') {
            ...
            $user_update = "UPDATE `admins` SET `name`='$name', `email`='$email', $password_update `phone`='$phone', `admin_level`='$admin_level', `percentage`='$percentage' WHERE `id`='$user_id'";
    
            $result_query_update = mysqli_query($conn, $user_update);
    
            if ($result_query_update) {
                echo "<script>alert('Data updated successfully!')</script>";
                echo "<script>window.open('index.php_L1?view_admin_L1','_self')</script>";
            } else {
        ...
        <div class="bg-secondary rounded h-100 p-4">
            ...
            <div>
                <h6 class="mb-4 text-primary">Agents Update
                    <a href="index.php?view_admin" class="btn btn-primary btn-sm float-end">Back</a>
                </h6>
            </div>
            <form action="" method="POST" class="mb-2">
                <div class="row">
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="text" name="email" value="<?php echo $referal_code; ?>" class="form-control" readonly/>
                    </div>
