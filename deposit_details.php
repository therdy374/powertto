...
<section class="container">
    ...
    <div class="contents">
        ...
        <div class="inner-contents">
            ...
            $sql2 = "SELECT COUNT(*) AS row_count FROM users_request WHERE user_id = '$user_id' AND DATE(date_message) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)";
            ...
            if ($query_run2->num_rows > 0) {
                ...
                $sql = "SELECT * FROM users_request WHERE user_id = $user_id AND DATE(date_message) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) ORDER BY `date_message` DESC LIMIT $start_index, $results_per_page";
                        ...
                        <table class="table-comm w100p" cellpadding="0" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    $days_old = (time() - strtotime($row['date_message'])) / (60 * 60 * 24);
                                    $reminder_class = $days_old > 30 ? 'reminder' : '';
                                    $hide_class = $row['hide'] == 1 ? 'hide-row' : '';

                                    echo "<tr class='$reminder_class $hide_class' data-id='{$row['req_id']}'>";
                                    echo '
                                        <td>' . $num-- . '</td>
                                        <td class="t-c">' . date('Y.m.d(D) h:i:s(A)', strtotime($row['date_message'])) . '</td>
                                        <td>' . $row["name"] . '</td>
                                        <td class="t-c">
                                            <span class="t-red">' . $row["message"] . ' ₩</span>
                                        </td>
                                        <td class="t-c">' . $row["status"] . '</td> 
                                        <td class="t-c t-blue"><button onclick="toggleHide(' . $row['req_id'] . ', ' . $row['hide'] . ')">지우다</button></td> 
                                    </tr>';
                                }
                                ?>
                            </tbody>
                    ...
                    <script>
                        function toggleHide(id, currentHideStatus) {
                            ...
                            if (row) {
                                ...
                                xhr.open('POST', 'update_hide_deposit.php', true);
                                ...
                                xhr.send('req_id=' + id + '&hide=' + newHideStatus);
                            }
                        }
                    </script>
                    <style>
