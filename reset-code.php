...
function send_password_reset($get_name, $get_email, $token)
{
    ...
    $mail->Username   = 'geronimo.t0324@gmail.com';
    $mail->Password   = 'vorg uqnr iqyn xlzk';

    $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
    $mail->Port       = 587;


    $mail->setFrom('geronimo.t0324@gmail.com', $get_name);
    ...
    $email_template = "
        ...
        <br/><br/>
        <a href='http://localhost/phpproject/powertto/password_change.php?token=$token&email=$get_email'> Click Me </a>
    ";
    ...
    if (!empty(trim($_POST['email']))) {
        ...
        if (mysqli_num_rows($checkemail_run) > 0) {
            ...
            if ($update_token_run) {
                ...
                // echo "<script>alert('We e-emailed you a password reset! Please check your email')</script>";
                // echo "<script>window.location.href=reset-password.php</script>";
                redirect('password_find.php', 'We e-emailed you a password reset link! Please check your email');
                exit(0);
            } else {
                redirect('password_find.php', 'Something went wrong!');
                exit(0);
            }
        } else {
            redirect('password_find.php', 'Your email is not valid!');
            exit(0);
        }
    } else {
        redirect('password_find.php', ' Please enter the email field!');
        exit(0);
    }
}
